    ...
    async continueRun(runId: string, workerId: string) {
      ...
      try {
        ...
        const computerInstruction = graphical
          ? "You have a persistent computer. Use computer_observe and computer_act for its visible desktop, including browsers and installed applications. Use open_path and launch_app to open graphical files, URLs, and applications. Use the file tools and shell for precise filesystem and terminal work. To read something from the web, fetch it with shell — curl, wget, python3 and jq are installed — and only drive the browser when a page needs signing in to, clicking through, or rendering to be understood. Pick one of those two approaches and stay with it: alternating between the shell and the desktop for the same goal makes no progress and ends the turn early. If an approach fails twice, say what failed and try the other one, rather than varying the same command again. On a Team Computer you have your own screen; other Team bots may run at the same time on theirs. Another user may interact with your screen while you run, so re-observe when it may have changed."
